...
read -rp "你需要TAG标签吗(Y/N): " chrony_install
    ...
    [yY][eE][sS] | [yY])
        ...
        docker run --name nginx-mtproxy -d -e adtag="$tag" -e secret="$secret" -e domain="$domain" -e provider=3 -p 80:80 -p $port:$port ellermister/mtproxy:latest
        ;;
    *)
...
echo -e "正在安装nginx-mtproxy... "
docker run --name nginx-mtproxy -d -e secret="$secret" -e domain="$domain" -e provider=3 -p 80:80 -p $port:$port ellermister/mtproxy:latest
        ;;
    esac
